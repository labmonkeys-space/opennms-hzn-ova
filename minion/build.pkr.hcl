...
build {
  source "source.qemu.opennms-horizon-minion-amd64" {
    name             = "onms-hzn-minion"
  ...
  provisioner "ansible-local" {
    playbook_file = "../ansible/hzn-minion-deployment.yml"
  ...
  post-processors {
    ...
    post-processor "shell-local" {
      inline = ["qemu-img convert image/packer-opennms-horizon-minion-amd64 -O vmdk -o adapter_type=lsilogic,subformat=streamOptimized,compat6 image/onms-hzn-minion-1.vmdk"]
    }
  }
}
